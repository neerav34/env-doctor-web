const FEATURES = [
  ...
  },
  {
    ...
    link: 'https://github.com/marketplace/actions/env-doctor',
  },
  {
    icon: '📄',
  ...
  },
  {
    ...
    link: 'https://marketplace.visualstudio.com/items?itemName=neeravjha.env-doctor',
  },
]
          ...
          {FEATURES.map(f => {
            const inner = (
              <>
                ...
                <h3 className={`font-mono font-semibold text-zinc-100 mb-2 transition-colors ${f.accent}`}>
                  {f.title}
                  {f.link && <span className="ml-2 text-xs font-normal opacity-50">↗</span>}
                </h3>
            ...
            return f.link ? (
              <a
                ...
                className="group p-6 rounded-xl bg-zinc-900 border border-zinc-800 card-glow transition-all duration-200 block"
              >
                {inner}
              </a>
            ) : (
